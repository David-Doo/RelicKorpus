...
function normalizeState(currentState) {
  currentState.users = Array.isArray(currentState.users) ? currentState.users : [];
  currentState.files = Array.isArray(currentState.files) ? currentState.files : [];
  currentState.permissionRequests = Array.isArray(currentState.permissionRequests) ? currentState.permissionRequests : [];
  currentState.lockRequests = Array.isArray(currentState.lockRequests) ? currentState.lockRequests : [];
  currentState.identityRequests = Array.isArray(currentState.identityRequests) ? currentState.identityRequests : [];
  currentState.tasks = Array.isArray(currentState.tasks) ? currentState.tasks : [];
  currentState.chats = Array.isArray(currentState.chats) ? currentState.chats : [];
  currentState.rules = Array.isArray(currentState.rules) ? currentState.rules : [];
  currentState.ruleRequests = Array.isArray(currentState.ruleRequests) ? currentState.ruleRequests : [];
...
function hasAccess(file, user) {
  ...
  if (["Enforcer", "Admin", "Core"].includes(canonicalRank(user.rank))) return true;
...
function canManageChatLimit(user) {
  return !!user && ["Admin", "Core"].includes(canonicalRank(user.rank));
}
